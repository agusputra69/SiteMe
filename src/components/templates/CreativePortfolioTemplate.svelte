...
<div
	class="min-h-screen bg-gradient-to-br from-purple-50 via-pink-50 to-indigo-50 dark:from-gray-900 dark:via-purple-900 dark:to-indigo-900"
>
	<!-- Navigation -->
	<nav
		class="fixed top-6 left-1/2 transform -translate-x-1/2 z-50 bg-white/80 dark:bg-gray-800/80 backdrop-blur-lg rounded-full px-6 py-3 shadow-lg"
	>
		<div class="flex items-center space-x-6">
			<a
				href="#home"
				class="text-gray-700 dark:text-gray-300 hover:text-{theme.primary} transition-colors font-medium"
				>Home</a
			>
			<a
				href="#about"
			...
			<a
				href="#contact"
				class="bg-{theme.primary} text-white px-4 py-2 rounded-full hover:bg-{theme.secondary} transition-colors font-medium"
				>Contact</a
			>
		</div>
	</nav>
	...
	>
		<div class="text-center max-w-4xl mx-auto">
			...
			<div class="mb-8">
				...
				{:else}
					<div
						class="w-40 h-40 bg-gradient-to-br from-{theme.primary} to-{theme.secondary} rounded-full flex items-center justify-center text-5xl font-bold text-white shadow-2xl border-4 border-white mx-auto"
					>
						{avatarInitial}
					</div>
				{/if}
			...
			{:else}
				<h1
					class="text-5xl sm:text-7xl font-bold bg-gradient-to-r from-{theme.primary} to-{theme.secondary} bg-clip-text text-transparent mb-6"
				>
					{profileData.name}
				</h1>
			{/if}
			...
			{#if profileData.workExperience && profileData.workExperience.length > 0}
				<p class="text-2xl sm:text-3xl text-gray-700 dark:text-gray-300 mb-8 font-light">
					...
					{#if profileData.workExperience[0].company}
						<br /><span class="text-{theme.primary} font-semibold"
							>@ {profileData.workExperience[0].company}</span
						>
					{/if}
				</p>
			{/if}
			...
			{:else}
				<p
					class="text-lg sm:text-xl text-gray-600 dark:text-gray-300 leading-relaxed max-w-3xl mx-auto mb-12"
				>
					{profileData.about}
				</p>
			{/if}
			...
			<div class="flex flex-col sm:flex-row gap-6 justify-center mb-12">
				{#if profileData.contact?.email}
					<a
						...
						class="px-8 py-4 bg-gradient-to-r from-{theme.primary} to-{theme.secondary} text-white font-semibold rounded-full hover:shadow-2xl transition-all duration-300"
					>
						Let's Work Together
					</a>
				{/if}
				<button
					class="px-8 py-4 border-2 border-{theme.primary} text-{theme.primary} font-semibold rounded-full hover:bg-{theme.primary} hover:text-white transition-all duration-300"
				>
					Download Resume
				</button>
			</div>
				...
				<div class="flex justify-center space-x-6">
					{#each profileData.links as link}
						<a
							...
							class="w-12 h-12 bg-white/80 rounded-full flex items-center justify-center text-{theme.primary} hover:bg-{theme.primary} hover:text-white transition-all duration-300 shadow-lg"
						>
	...
	<section id="about" class="py-20 px-4 sm:px-6 lg:px-8">
		<div class="max-w-6xl mx-auto">
			<h2 class="text-4xl sm:text-5xl font-bold text-center text-gray-900 dark:text-white mb-16">
				About <span
					class="bg-gradient-to-r from-{theme.primary} to-{theme.secondary} bg-clip-text text-transparent"
					>Me</span
				>
			</h2>
			...
			<div class="grid lg:grid-cols-2 gap-16 items-center">
						...
						<div>
							...
							<div class="grid grid-cols-2 sm:grid-cols-3 gap-4">
								{#each profileData.skills as skill}
									<div
										class="bg-white/60 rounded-xl p-4 text-center shadow-lg border border-white/20"
									>
				...
				<div
					class="bg-gradient-to-br from-{theme.primary}/20 to-{theme.secondary}/20 rounded-3xl p-8"
				>
					...
					{#each profileData.workExperience as experience}
						<div class="bg-white dark:bg-gray-800 rounded-2xl p-8 shadow-lg">
							<div class="flex flex-col sm:flex-row sm:items-start sm:justify-between mb-4">
								...
								<span
									class="px-3 py-1 bg-{theme.accent} text-{theme.primary} rounded-full text-sm font-medium mt-4 sm:mt-0"
								>
									{experience.type}
								</span>
							</div>
			...
			<div class="max-w-7xl mx-auto">
				...
				<div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
					{#each profileData.projects as project}
						<div
							class="bg-white dark:bg-gray-800 rounded-3xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300"
						>
							{#if project.image}
								<img src={project.image} alt={project.title} class="w-full h-56 object-cover" />
							{:else}
								<div
									class="w-full h-56 bg-gradient-to-br from-{theme.primary} to-{theme.secondary} flex items-center justify-center"
								>
								...
								<div class="flex space-x-3">
									{#if project.liveUrl}
										<a
											...
											target="_blank"
											class="flex-1 bg-gradient-to-r from-{theme.primary} to-{theme.secondary} text-white text-center py-3 rounded-xl font-medium"
											>Live Demo</a
										>
									{/if}
									{#if project.githubUrl}
										<a
											...
											target="_blank"
											class="flex-1 border-2 border-{theme.primary} text-{theme.primary} text-center py-3 rounded-xl font-medium"
											>View Code</a
										>
									{/if}
								</div>
	...
	<section
		id="contact"
		class="py-20 px-4 sm:px-6 lg:px-8 bg-gradient-to-br from-{theme.primary}/10 to-{theme.secondary}/10"
	>
