<script lang="ts">
	...
	import { ArrowLeft, Mail, Lock, Eye, EyeOff, Github, User as UserIcon } from 'lucide-svelte';
	...
	onMount(async () => {
		...
		try {
			...
			// Session check failed on signup page
		}
	});
				...
				<div>
					...
					<div class="relative">
						...
						>
							{#if showPassword}
								<EyeOff
									class="h-5 h-5 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300"
								/>
							{:else}
				...
				<div class="mt-6">
					...
					>
						{#if loading}
							<div class="flex items-center justify-center">
								<div
									class="animate-spin rounded-full h-5 w-5 border-b-2 border-gray-600 dark:border-white mr-2"
								/>
								Connecting...
							</div>
						{:else}
