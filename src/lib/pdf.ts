...
export async function extractTextFromPDF(file: File): Promise<string> {
  try {
    ...
    if (result.length === 0) {
      throw new Error('No text could be extracted from the PDF. The document may be scanned, password-protected, or corrupted.');
    }
    
    if (result.length < 50) {
      throw new Error('Very little text extracted. The PDF may be scanned, password-protected, or not a resume document.');
    }
    
    if (pagesWithText === 0) {
      throw new Error('No pages contained readable text. The PDF may be image-based or password-protected.');
    }
    
    // Check for common issues
    if (result.includes('password') || result.includes('Password')) {
      throw new Error('PDF appears to be password-protected. Please remove the password and try again.');
    }
    ...
    if (error instanceof Error) {
      ...
      if (message.includes('No text could be extracted')) {
        throw new Error('No text could be extracted from the PDF. This usually means the document is scanned or image-based. Please use a text-based PDF.');
      }
      
      if (message.includes('PDF.js library is not available')) {
        throw new Error('PDF processing library is not loaded. Please refresh the page and try again.');
      }
...
export function isPDFLikelyResume(text: string): boolean {
  const resumeKeywords = [
    ...
    'resume', 'cv', 'curriculum vitae', 'professional', 'career'
  ];
