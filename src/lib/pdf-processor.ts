...
export function extractBasicResumeData(text: string): BasicResumeData {
	...
	// Post-process and clean data
		...
		handleError(error, {
			...
			action: 'extractBasicResumeData',
			userMessage: 'Failed to process PDF content'
		});
...
function identifySections(lines: string[]): Map<string, string[]> {
	...
	const sectionPatterns = {
		experience:
			/^(experience|work\s+experience|professional\s+experience|employment|career|work\s+history|pengalaman)$/i,
...
function parseSkillsSection(content: string[], result: BasicResumeData): void {
	...
	const softSkills =
		/\b(leadership|communication|teamwork|problem solving|analytical|creative|adaptable|organized|time management|project management|critical thinking|collaboration|presentation|negotiation|customer service|mentoring|training)\b/i;

	const languages =
		/\b(english|indonesian|mandarin|spanish|french|german|japanese|korean|arabic|portuguese|russian|italian|dutch|hindi|thai|vietnamese|bahasa)\b/i;

	const tools =
		/\b(photoshop|illustrator|figma|sketch|canva|office|excel|word|powerpoint|outlook|slack|trello|jira|confluence|notion|zoom|teams|salesforce|hubspot|mailchimp|google analytics|tableau|power bi)\b/i;
	...
	for (const line of content) {
		...
		// Extract skills from longer text
			...
			for (const word of words) {
				...
				if (cleanWord.length > 2) {
					if (
						technicalSkills.test(cleanWord) ||
		...
		for (let skill of skills) {
			...
			if (technicalSkills.test(lowerSkill)) {
				...
				if (
					!lowerSkill.match(
						/^(and|or|the|a|an|in|on|at|to|for|with|by|from|of|as|is|are|was|were|be|been|have|has|had|do|does|did|will|would|could|should|may|might|can|must)$/
					)
				) {
...
function cleanAndValidateData(data: BasicResumeData): void {
	...
	if (data.name) {
		...
		data.name = data.name
			.replace(/^(resume|cv|curriculum vitae|profile|about|summary)[:\s]*/i, '')
			.trim();
	...
	data.certifications = [
		...new Set(data.certifications.filter((cert) => cert && cert.trim().length > 2))
	];
